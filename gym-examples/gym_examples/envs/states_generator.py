...
#     'number_of_main_pages': 4,
#     'number_of_bugs_in_states': 5,
#     'number_of_bugs_in_actions': 5
# }
#
...
params = {
    'tree_depth': 2,
    'max_number_of_subpages': 2,
    'number_of_main_pages': 3,
    'number_of_bugs_in_states': 3,
...
class SystemGenerator:
    def __init__(self, tree_depth=params['tree_depth'],
        ...
        self.system = self.generate_system()
        bug_creator = BugCreator(system=self.system)
        self.list_of_bugs = bug_creator.create_bugs_randomly(number_of_bugs_in_states=2, number_of_bugs_in_actions=3, list_of_all_actions=self.list_of_actions)
        self.create_file_with_scheme_code(self.list_of_bugs)
...
class BugCreator:
    ...
    def create_bugs_randomly(self, number_of_bugs_in_states, number_of_bugs_in_actions, list_of_all_actions):
        list_of_states = self.system.keys()
        list_of_states_with_bugs = random.sample(list_of_states, number_of_bugs_in_states)
        # list_of_actions_with_bugs = random.sample(
        #     [key for value in self.system.values() for key in value['actions'].keys()],
        #     number_of_bugs_in_actions)
        list_of_actions_around_bugged_states = []
        for bugged_state in list_of_states_with_bugs:
            list_of_actions_around_bugged_states += list(filter(lambda x: bugged_state in x, list_of_all_actions))
        list_of_actions_with_bugs = random.sample(list_of_actions_around_bugged_states, number_of_bugs_in_actions)
        self.list_of_bugs = list_of_states_with_bugs + list_of_actions_with_bugs
        self.print_all_bugs()
        return self.list_of_bugs
